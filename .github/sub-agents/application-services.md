...
* **Do** define DTOs based on the [DTO best practices](.github/sub-agents/data-transfer-objects.md).

* **Do** use Abp AutoMapper for mapping attribue between Entities and DTOs, instead of defining them manually.
* **Do** AutoMapper configuration (e.g., `Profile` classes) will also be placed in the `Application/[Application name]Management` folder.
* **Do** To efficiently copy values between objects, especially when multiple properties are involved in the assignment, you should utilize ABP Object Mapping, which is typically powered by tools like AutoMapper. This approach helps you avoid tedious manual adjustments every time you add or modify properties, ensuring the mapping process is automatic, consistent, and maintainable."
  
Reference [ABP Object Mapping ](../sub-agents/object-mapper.md)
