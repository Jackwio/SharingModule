...
Every aggregate root is also an entity. So, these rules are valid for aggregate roots too unless aggregate root rules override them.Create Entity in the `Domain/Models` folder.

- **Do** define entities in the **domain layer**.
- **Do** having an `IsDeleted` property must implement the `ISoftDelete` interface.
- **Do** having an `TenantId` property must implement the `ITenant` interface.
